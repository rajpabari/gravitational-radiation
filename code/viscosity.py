# %%
from jax import jacobian, random, config, debug, lax
...
step_size = 1e-2  # step size for the ODE solver


eps = 1e-3  # initial perturbation
e = 0.94  # initial eccentricity
a_0 = jnp.array(
    [
        (1 - e**2) ** (-1 / 6) * (1 + eps),
        (1 - e**2) ** (-1 / 6) / (1 + eps),
...
T = 3  # time upper bound -- ODE will be solved for t \in [0, T]
a_0
...
def _Sigma(sign: Int, a: Array, A: Array) -> Float:
    ...
    lax.cond(
        jnp.any(Sigma < 0),  # scalar boolean condition
        lambda _: debug.breakpoint(),
        lambda _: None,
        operand=None,
    )
    # if Sigma.item() < 0.0:
    #     debug.print("{x}", x=Sigma)
    return Sigma
...
def scalar_coeffs(t, r, args):
    nu = args[0]
    return nu * _b(r)


# def inverse_coeffs(t, z, args):
#     nu = args[0]
#     return -nu * _b(1 / z) * z**2


term = ODETerm(coeffs)
# inverse_term = ODETerm(inverse_coeffs)
solver = Dopri5()
...
# %%
# # jnp.isnan(ys[:, 0]).shape
# # plt.imshow(jnp.isnan(ys).T, aspect="auto", cmap=plt.cm.gray, interpolation="nearest")
# plt.figure()
# b_sol = jnp.array([_b(1 / z) for z in a_sol[~jnp.isnan(a_sol).any(axis=1)]])
# plt.plot(ts[~jnp.isnan(a_sol[:, 0])], b_sol[:, 0], label="da_1/dt")
# plt.plot(ts[~jnp.isnan(a_sol[:, 0])], b_sol[:, 1], label="da_2/dt")
# plt.plot(ts[~jnp.isnan(a_sol[:, 0])], b_sol[:, 2], label="da_3/dt")
# plt.xlabel("Time")
# plt.ylabel("da_i/dt")
# plt.title("b coefficients over time")
# plt.legend()
# plt.show()
...
plt.figure()
# plt.xlim(0, T)
plt.plot(ts, a_sol[:, 0], label="a1")
plt.plot(ts, a_sol[:, 1], label="a2")
plt.xlabel("Time")
plt.ylabel("y")
...
# %%
plt.figure()
plt.plot(ts, a_sol[:, 0] * a_sol[:, 1] * a_sol[:, 2], label="mass")
plt.xlabel("Time")
plt.ylabel("y")
